...
class TestStringCalculatorAdd < Test::Unit::TestCase
  ...
  def setup
    @string_calculator = StringCalculator.new InputParser.new, NumbersValidator.new
  end
  ...
  def test_with_minus_two_a_exception_is_raised
    exception = assert_raises(RuntimeError) { @string_calculator.add("-2") }
    assert_equal "negatives not allowed: -2", exception.message
  end

  def test_with_minus_two_semicolon_and_minus_three_a_exception_is_raised
